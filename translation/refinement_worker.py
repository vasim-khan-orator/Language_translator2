...
#     """
#     Context-aware translation refinement worker.

#     Refinement is intentionally low-priority. It is delayed so
#     the system does not immediately spend another Ollama request
#     after every final translation.

#     Only one refinement request is allowed to use Ollama at a
#     time. For the same turn, only the latest refinement request
#     is allowed to update the conversation.
#     """
...
#             # Refinement is deliberately single-flight because it
#             # competes with final/chunk translation for the same
#             # remote Ollama model.
...
#             # Use an Event instead of time.sleep() so shutdown can
#             # cancel the delay immediately. This also prevents the
#             # old "cannot schedule new futures after interpreter
#             # shutdown" path during application exit.
...
#                 # Normal during shutdown when the executor has
#                 # already been closed. Do not report this as an
#                 # application error.
#                 if self._running:
#                     raise
#                 return
